class DisputeDocument < ActiveRecord::Base
 
  # Environment-specific direct upload url verifier screens for malicious posted upload locations.
  DIRECT_UPLOAD_URL_FORMAT = %r{\Ahttps:\/\/s3-us-west-2\.amazonaws\.com\/adjudicate#{!Rails.env.production? ? "-test" : ''}\/(?<path>uploads\/.+\/(?<filename>.+))\z}.freeze
  ...
  def self.transfer_and_cleanup(id)
    document = DisputeDocument.find(id)
  ...
  def queue_processing
    DisputeDocument.delay.transfer_and_cleanup(id)
  end
